...
void EventManager::commandProcessType80ClickInDungeonView(int16 posX, int16 posY) {
	...
	if (_vm->_dungeonMan->_squareAheadElement == kDMElementTypeDoorFront) {
		...
		if (_vm->_championMan->_leaderEmptyHanded) {
			...
			if (DOOR_hasButton(junkPtr) && _vm->_dungeonMan->_dungeonViewClickableBoxes[kDMViewCellDoorButtonOrWallOrn].isPointInside(posX, posY - 33)) {
				_vm->_stopWaitingForPlayerInput = true;
	...
	if (_vm->_championMan->_leaderEmptyHanded) {
		...
		byte *junkPtr = _vm->_dungeonMan->getThingData(thingHandObject);
